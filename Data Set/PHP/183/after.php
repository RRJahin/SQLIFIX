...
try {
    ...
    $stmt2 = $conn->prepare("UPDATE ? SET name = ? WHERE id = ? and email=?");
    $stmt1->bind_param("ssis",$wpdb->myTable, $_POST['name'], $_POST['age'], $id);
    $stmt2->bind_param("ssis",$wpdb->myTable, $_POST['name'], $_SESSION['id'], $email);
    $stmt1->execute();
    $stmt2->execute();
    $stmt1->close();
    $stmt2->close();
