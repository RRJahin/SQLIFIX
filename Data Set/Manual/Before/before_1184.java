...
protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try {
        ...
        ps = agreg.con.createStatement();
        System.out.println("trying again");
        rs = ps.executeQuery("select count(*) from agent_details where location="+location);
        if (rs.next()) {
            count = rs.getInt(1);
            rs.close();
            ps.close();
        ...
        if (count != 0) {
            ...
            ps = agreg.con.createStatement("select agn_id from agent_details where email_id = ? and location=? and phone_no=?");
