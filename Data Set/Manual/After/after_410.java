...
public Reply vote(String jsonString) {
    ...
    try (Connection connection = dataSource.getConnection()) {
        ...
        String query = new StringBuilder("UPDATE ").append(tableName).append(" SET ").append(column).append(" = ").append(column).append(" + 1, points = points + ").append(Integer.toString(vote)).append(" WHERE id = ?").toString();
