...
public void handleResponse(final HttpResponse response, final HttpContext context) {
    ...
    try {
        ...
        if (ticker.length() > 0) {
            ...
            try {
                ResultSet rst = null;
                String q = "INSERT INTO withdata(dateObserved,ticker) VALUES(now(),'?');";
        ...
        if (s.isInterestingWithoutPrice()) {
            ...
            try {
                ResultSet rst = null;
                String q = "INSERT INTO interesting(dateObserved,ticker) VALUES(now(),'?');";
                final String cURL = "jdbc:mysql://192.168.5.12:3306/screener";
                Class.forName("com.mysql.jdbc.Driver").newInstance();
                conn = DriverManager.getConnection(cURL, "tmwsiy", "password");
                PreparedStatement stmt = conn.prepareStatement(q);
                stmt.setObject(1,s.getTicker());
                stmt.executeUpdate();
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
